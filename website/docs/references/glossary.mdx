---
id: glossary
title: 'Lagune Glossary: SDH, Charter, Findings, and Controls'
sidebar_label: Glossary
description: 'Plain-language definitions of every Lagune and Security-Driven Hardening term: SDH, charter, detect, finding, control, harden, verify, stand down, closability, sub-skills, and specializations.'
keywords:
...
The methodology Lagune implements: a context-aware, blue-team convention that detects a system's context, triages the risks that context carries, and guides the fixes. The intelligence lives in the spec, not in the user, so a developer and a non-developer are served through the same flow.
...
The universal command `/lagune` that secures work as it is written. It takes any prompt at any stage, builds what you asked, and makes it safe by default before handing it back, guided by the charter when one exists. It writes nothing to Lagune's tracking, so its only product is the safe code. Together with the charter (before) and the Blue Team flow (after), it is how Lagune protects during the build.
...
<Faq
  items={[
    ...
    {
      question: 'What is a charter in Lagune?',
      answer:
    ...
    {
      question: 'What does it mean to stand down a finding?',
      answer:
        'When verify proves a risk closed, the finding is stood down: removed from the whole chain with your confirmation, so it stops being reprocessed.',
    },
  ]}

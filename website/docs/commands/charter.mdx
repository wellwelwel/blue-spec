...
🧬 Tell **Blue Spec** what your project is, and it turns that into your security rules (or, with no description, works them out from the project alone).
...
```
  ...
  <TabItem value="propose" label="Let Blue Spec propose">
...
**Blue Spec** takes your description or PRD as the direction and works out what it implies for security, then reads the project to go further than you named. For example:
...
Your description and the project add up, so while your description sets the intent, reading the dependencies, the README, and the configuration surfaces what you did not mention. If something important is still unclear, **Blue Spec** asks a short, plain question rather than guess.

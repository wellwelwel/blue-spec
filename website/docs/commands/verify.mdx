...
If the risk is not closed, verify says so plainly and sends you back to [`/bluespec.harden`](/docs/commands/harden), since the fix is still open.
...
Verify builds on [`/bluespec.harden`](/docs/commands/harden).
:::

...
description: Move a legacy .bluespec/ install to Lagune with a single command, keeping your charter, artifacts, tracking, and own sub-skills.
keywords:
  - migrate
  - lagune migrate
  - legacy install
...
:::note[For projects installed under the legacy name]
...
:::info[Your memories and specializations are never lost]
Only the old name inside them changes, never the content. The one exception, as in [`update`](/docs/get-started/install#update): a built-in you refined is reset to the shipped version, still recoverable from your git history.
:::
...
5. Refreshes the templates, hooks, and built-in sub-skills to the installed version, the same way [`update`](/docs/get-started/install#update) does.

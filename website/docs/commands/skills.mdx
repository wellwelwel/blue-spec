---
id: skills
title: 'Skills: On-Demand Security Modules'
...
You choose which categories to install when you set up **Lagune**, and change them any time with `add` and `remove`. See [**Install**](/docs/get-started/install#specializations) for the `--skills` flag. Each installed sub-skill is a file under `.lagune/skills/`.
...
<Faq
  items={[
    {
      question: 'What is a Lagune sub-skill?',
      answer:
    ...
    {
      question: 'Are Lagune skills loaded by default?',
      answer: 'No. They load only on demand, for what the project actually is.',
    },
    {
      question: 'How do I load a sub-skill directly?',
      answer: 'Import it into any prompt with @.lagune/skills/<name>.md.',
    },
  ]}
/>

---
id: jwt
title: 'jwt hook: Detect unpinned JWT verification'
sidebar_label: jwt
description: Sweep a codebase for JWT verification without an algorithm pin, or score a single call, from the command line.
---
...
:::tip
The hook decides only the algorithm pin: the claim, key, and flow checks are judgment the [`federation` sub-skill](/docs/commands/skills) still covers.
:::

<Faq
  items={[
    {
      question: 'What is JWT algorithm confusion?',
      answer:

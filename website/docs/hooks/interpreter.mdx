---
id: interpreter
title: 'interpreter hook: Flag dynamic-code-execution sinks'
sidebar_label: interpreter
description: Sweep a codebase for dynamic-code-execution sinks across languages, or score a single snippet, from the command line.
---
...
The `interpreter` hook flags **dynamic-code-execution sinks**: constructs that turn data into running code or hand it to a shell, from `eval` to native deserialization. It is language-aware, reading each file by its own rules, and it is the deterministic engine behind the [`interpreter` sub-skill](/docs/commands/skills), which you can run yourself in **scan** or **check** mode.
...
```
  ...
  <TabItem value="check" label="Check a snippet">
...
:::danger[Read the flagged code, never run it!]
...
:::tip
The hook guarantees the floor (these sinks exist and where): whether an untrusted value actually reaches one is the judgment the [`interpreter` sub-skill](/docs/commands/skills) covers.
:::

<Faq
  items={[
    {
      question: 'Does the interpreter hook confirm a vulnerability?',
      answer:
    ...
    {
      question: 'Which languages does the interpreter hook cover?',
      answer:

---
id: network
title: 'network hook: Score URLs for SSRF'
sidebar_label: network
description: Score one or many fetch destinations for SSRF, straight from the command line.
---
...
The `network` hook decides whether a server-side fetch destination is safe from **SSRF** (a request that can be aimed at internal services, cloud metadata, or private hosts). It is the deterministic engine behind the [`network` sub-skill](/docs/commands/skills), and you can run it yourself.
...
<Tabs>
  <TabItem value="one" label="One destination" default>
...
```
  ...
  <TabItem value="many" label="Several destinations">
...
This is the same check the [`network` sub-skill](/docs/commands/skills) runs to prove a destination filter holds.
:::

<Faq
  items={[
    {
      question: 'How do I test a URL for SSRF?',
      answer:
    ...
    {
      question: 'What is a parser-divergent URL?',
      answer:

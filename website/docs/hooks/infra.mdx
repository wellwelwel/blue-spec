---
id: infra
title: 'infra hook: Detect IaC misconfiguration'
sidebar_label: infra
description: Sweep Terraform, Dockerfiles, and GitHub Actions for misconfiguration, or score a single snippet, from the command line.
---
...
:::tip
The hook catches the blunt, decidable cases: absent encryption, `pull_request_target` exposure, and the right IAM scope are judgment the [`infra` sub-skill](/docs/commands/skills) still covers.
:::

<Faq
  items={[
    {
      question: 'What infrastructure files does the infra hook read?',
      answer:

---
id: crypto
title: 'crypto hook: Detect weak cryptography'
sidebar_label: crypto
description: Sweep a codebase for weak cryptographic primitives, or score a single snippet, from the command line.
---
...
The `crypto` hook flags **weak cryptographic primitives** at the call site: a deprecated cipher, an insecure mode, or a broken digest. It is **language-aware**, reading each file by its own rules so a Python construct is never flagged on a Rust file, and it is the deterministic engine behind the [`crypto` sub-skill](/docs/commands/skills), which you can run yourself in **scan** or **check** mode.
...
:::tip
The hook guarantees the floor (these constructs exist): key management, IV reuse, and RSA padding are judgment the [`crypto` sub-skill](/docs/commands/skills) still covers.
:::

<Faq
  items={[
    {
      question: 'How do I scan a codebase for weak cryptography?',
      answer:
    ...
    {
      question: 'Does the crypto hook flag Math.random()?',
      answer:

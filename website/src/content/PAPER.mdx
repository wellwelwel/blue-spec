...
- **Weslley Araújo**
- **Philosophy Paper (Experimental), 2026.**
...
**Blue Spec** is the reference implementation. It realizes SDH across 37 AI coding agents and proves the methodology is buildable and portable. This is experimental work, run only in controlled environments ahead of public release, offered as a convention others can adopt.
...
| --- | ------- | -------------- | ------------------------------------------------ | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| 1   | charter | policy         | the project, and the user's description if given | the charter                                                          | Principles carry the risk they prevent, in plain language.                            |
...
| 5   | verify  | verify         | the code, and the hardening record               | each verdict onto the hardening record, and closure across the chain | Verification is the confrontation of code against record, statically. Never softened. |
...
>
> The same finding name carries through the detect map, the plan, and the hardening record, and verify writes its verdict onto that record. That shared name is its identity (`§X`).
...
| ------- | ------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| charter | SDH reads the project itself and proposes principles from it. | A description (or the user's own rules) sets direction, broadened by SDH's read of the project. |
...
5. **verify** reads each spot and confronts code against record. The upload control holds: the handler detects the real type from content, enforces a size limit, and refuses the rest. Verdict: risk closed. Verify stands the upload finding down across the whole chain.
